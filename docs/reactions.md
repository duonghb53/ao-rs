...
| `agent-idle` | Agent has been idle past the threshold | notify human |
...
pub enum EventPriority { Urgent, Action, Warning, Info }
...
pub struct ReactionConfig {
    ...
    pub escalate_after: Option<EscalateAfter>,   // Count(u32) | Duration(String)
    pub threshold: Option<String>,               // e.g. "10m" — parsed lazily per tick
    pub include_summary: bool,
}

pub enum EscalateAfter {
    Attempts(u32),
    Duration(String),   // raw string e.g. "10m", parsed lazily by parse_duration()
}
...
```yaml
# ao-rs.yaml  (project directory, generated by `ao-rs start`)
defaults:
  scm: github
  tracker: github
  agent: claude-code
...
- **Slack / email notifiers.** One crate each when needed. Desktop (`notifier-desktop`) and Discord (`notifier-discord`) are already implemented.
